...
const DEFAULT_UNITS = ["x (veces)","%","UF","CLP","USD","EUR","MM CLP","MM UF","veces"];
const DEFAULT_SECTORS = ["Energía","Retail","Inmobiliario","Industrial","Financiero","Salud","Educación","Tecnología","Telecomunicaciones","Agroindustria","Construcción","Transporte","Minería","Alimentos","Servicios"];
...
// Reusable creatable select: shows dropdown with existing options + "Crear nuevo"
function CreatableSelect({value,onChange,options,placeholder="Seleccionar...",width="100%"}){
  ...
  return(
    <div ref={ref} style={{position:"relative",width}}>
      ...
      {open&&<div style={{position:"absolute",top:"100%",left:0,right:0,zIndex:3000,background:"#fff",border:`1px solid ${L.border}`,borderRadius:5,boxShadow:"0 6px 20px rgba(0,0,0,0.12)",maxHeight:200,overflow:"auto"}}>
        {options.map(o=>(
        ...
        {!creating&&<div onClick={()=>setCreating(true)} style={{padding:"8px 12px",cursor:"pointer",fontSize:12,color:AZ,fontWeight:600,borderTop:`1px solid ${L.border}`,display:"flex",alignItems:"center",gap:6}}
          onMouseEnter={e=>e.currentTarget.style.background="rgba(55,81,114,0.04)"}
          onMouseLeave={e=>e.currentTarget.style.background="#fff"}>
          <span>＋</span> Crear nuevo...
        </div>}
        {creating&&<div style={{padding:"8px 10px",borderTop:`1px solid ${L.border}`,display:"flex",gap:6}}>
          <input autoFocus value={newVal} onChange={e=>setNewVal(e.target.value)}
            onKeyDown={e=>{if(e.key==="Enter"&&newVal.trim()){onChange(newVal.trim());setOpen(false);setCreating(false);setNewVal("");}if(e.key==="Escape"){setCreating(false);setNewVal("");}}}
            placeholder="Nuevo valor..." style={{...sinp,padding:"5px 8px",fontSize:11,flex:1}}/>
...
// Covenant row with unit field
function CovenantRow({r,isNew,onUpdate,onRemove,issuer,sinp,btnDanger,allUnits,setAllUnits}){
  ...
  return(
    <tr style={{background:isNew?"rgba(55,81,114,0.04)":"transparent"}}>
      <td style={{padding:"4px 5px",borderBottom:`1px solid ${L.border}`}}><input style={{...sinp(155)}} placeholder="ej: DFN/EBITDA" value={r.name} onChange={e=>onUpdate(r._id,"name",e.target.value)}/></td>
      ...
      <td style={{padding:"4px 5px",borderBottom:`1px solid ${L.border}`,minWidth:100}}>
        <CreatableSelect value={r.unidad||"x (veces)"} onChange={v=>{onUpdate(r._id,"unidad",v);if(!allUnits.includes(v))setAllUnits(p=>[...p,v]);}} options={allUnits} placeholder="Unidad" width="100px"/>
      </td>
      <td style={{padding:"4px 5px",borderBottom:`1px solid ${L.border}`}}><input style={{...sinp(78),borderColor:st==="breach"?L.danger:st==="warning"?L.warn:L.border}} placeholder="ej: 2,50" value={r.actualVal} onChange={e=>onUpdate(r._id,"actualVal",e.target.value)}/></td>
      <td style={{padding:"4px 5px",borderBottom:`1px solid ${L.border}`,whiteSpace:"nowrap"}}>{holgura!==null?<span style={{color:st==="breach"?L.danger:st==="warning"?L.warn:L.sub,fontWeight:500,fontSize:11}}>{fmtNum(holgura,r.unidad||"x")}</span>:<span style={{color:"#ccc"}}>—</span>}</td>
      <td style={{padding:"4px 5px",borderBottom:`1px solid ${L.border}`}}><button onClick={()=>onRemove(r._id)} style={{...btnDanger,padding:"3px 7px",fontSize:10}}>✕</button></td>
    </tr>
  );
}

// Covenant picker — no limits, no units
function CovenantPicker({issuer,onSelect,onClose}){
  ...
  useEffect(()=>{
    fetch("/api/issuers").then(r=>r.json()).then(list=>{
      ...
      list.forEach(iss=>{if(iss.id===issuer?.id)return;iss.covenants.forEach(c=>{const k=c.name+"||"+c.tipo+"||"+c.op;if(!seen.has(k)){seen.add(k);covs.push({name:c.name,tipo:c.tipo,op:c.op,_count:cnt[c.name]||1});}});});
  ...
  return(
    <div ref={ref} style={{position:"absolute",top:"100%",right:0,zIndex:2000,background:"#fff",border:`1px solid ${L.border}`,borderRadius:6,boxShadow:"0 8px 30px rgba(0,0,0,0.15)",width:340,maxHeight:280,display:"flex",flexDirection:"column"}}>
      <div style={{padding:"8px 10px",borderBottom:`1px solid ${L.border}`}}><input autoFocus placeholder="Buscar covenant..." value={search} onChange={e=>setSearch(e.target.value)} style={{width:"100%",border:`1px solid ${L.border}`,borderRadius:4,padding:"5px 8px",fontSize:12,outline:"none",fontFamily:"inherit"}}/></div>
      <div style={{overflowY:"auto",flex:1}}>
        <div onClick={()=>onSelect(null)} style={{padding:"8px 12px",cursor:"pointer",borderBottom:`1px solid ${L.border}`,background:"rgba(55,81,114,0.04)",display:"flex",alignItems:"center",gap:6}} onMouseEnter={e=>e.currentTarget.style.background="rgba(55,81,114,0.1)"} onMouseLeave={e=>e.currentTarget.style.background="rgba(55,81,114,0.04)"}>
          <span style={{fontSize:13,color:AZ}}>＋</span><span style={{fontSize:12,fontWeight:600,color:AZ}}>Crear covenant nuevo</span>
        </div>
        {filtered.length===0&&<div style={{padding:"14px 12px",color:L.sub,fontSize:12,textAlign:"center"}}>Sin resultados</div>}
        {filtered.map((c,i)=>(
          <div key={i} onClick={()=>onSelect(c)} style={{padding:"7px 12px",cursor:"pointer",borderBottom:`1px solid ${L.border}`,display:"flex",justifyContent:"space-between"}} onMouseEnter={e=>e.currentTarget.style.background="#f8f9fb"} onMouseLeave={e=>e.currentTarget.style.background="#fff"}>
            <div><div style={{fontSize:12,fontWeight:500}}>{c.name} <span style={{color:L.sub,fontWeight:400}}>{c.op}</span></div><div style={{fontSize:10,color:L.sub,marginTop:1}}>{c.tipo} · {c._count} emisor{c._count!==1?"es":""}</div></div>
          </div>
        ))}
      </div>
    </div>
...
function NewIssuerModal({onClose,onSuccess,allIssuers}){
  ...
  // Derive existing sectors from all issuers
  const existingSectors=useMemo(()=>{const s=new Set(DEFAULT_SECTORS);(allIssuers||[]).forEach(i=>{if(i.sector&&i.sector!=="Sin sector")s.add(i.sector);});return[...s].sort();},[allIssuers]);

  async function handleExtract(){if(!file)return;setLoading(true);setError("");try{const fd=new FormData();fd.append("pdf",file);fd.append("issuerName",name.trim()||"Nuevo emisor");fd.append("covenants",JSON.stringify([]));fd.append("detectMode","true");if(mode==="smart")fd.append("smartMode","true");const res=await fetch("/api/extract-pdf",{method:"POST",body:fd});const data=await res.json();if(!res.ok)throw new Error(data.error);const found=(data.covenants||[]).map(c=>({...c,tipo:c.tipo||"flujo",op:c.op||">=",lim:c.lim||null,limite:c.limite||"",unidad:c.unidad||"x (veces)",actualVal:c.act!=null?String(c.act):"",_id:Math.random(),_needsInput:c.actual===null}));setDetected(found);setFechaEEFF(data.fechaEEFF||"");setStep("review");}catch(e){setError(e.message);}finally{setLoading(false);}}
  ...
  async function handleSave(){if(!name.trim()){setError("El nombre es obligatorio");return;}setLoading(true);setError("");try{const listRes=await fetch("/api/issuers");const list=await listRes.json();const covenants=detected.filter(r=>r.name.trim()).map(r=>{const num=parseNum(r.actualVal);const lim=r.lim!==null&&r.lim!==undefined?r.lim:parseNum(r.limite);const unit=r.unidad||"x";if(isNaN(num))return{name:r.name,tipo:r.tipo,op:r.op,lim:isNaN(lim)?null:lim,limite:r.limite||"",unidad:unit,actual:null,act:null,holgura:null,actualStr:null,holguraStr:null};const holgura=r.op==="<="?lim-num:num-lim;return{name:r.name,tipo:r.tipo,op:r.op,lim:isNaN(lim)?null:lim,limite:r.limite||fmtNum(lim,unit),unidad:unit,actual:fmtNum(num,unit),act:num,holgura:isNaN(holgura)?null:holgura,actualStr:fmtNum(num,unit),holguraStr:fmtNum(holgura,unit)};});const newIssuer={id:name.trim().toLowerCase().replace(/\s+/g,"-").replace(/[^a-z0-9-]/g,"")+"-"+Date.now(),name:name.trim(),sector:sector||"Sin sector",clasificacion:clasificacion||"—",fechaEEFF:fechaEEFF||"—",covenants};const res=await fetch("/api/issuers",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify([...list,newIssuer])});if(!res.ok)throw new Error("Error guardando");onSuccess();}catch(e){setError(e.message);}finally{setLoading(false);}}
  ...
  const btnDanger={background:L.dangerBg,color:"rgb(170,40,50)",border:"none",borderRadius:4,cursor:"pointer",fontFamily:"inherit"};
  ...
  return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.45)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={e=>e.target===e.currentTarget&&onClose()}>
    <div style={{background:"#fff",borderRadius:8,width:"100%",maxWidth:step==="review"?820:480,maxHeight:"90vh",overflow:"auto",boxShadow:"0 20px 60px rgba(0,0,0,0.3)"}}>
      <div style={{padding:"20px 24px",borderBottom:`1px solid ${L.border}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div style={{fontSize:15,fontWeight:600,color:AZ}}>{step==="form"?"Nuevo emisor":step==="eeff"?"Cargar EEFF":`Covenants detectados — ${name}`}</div>
        <button onClick={onClose} style={{...btn("#f0f0f0","#666"),padding:"6px 12px"}}>✕</button>
      </div>
      <div style={{padding:24}}>
        {step==="form"&&(<div style={{display:"flex",flexDirection:"column",gap:14}}>
          ...
          <div><label style={{fontSize:11,color:L.sub,display:"block",marginBottom:5,fontWeight:600,textTransform:"uppercase",letterSpacing:0.5}}>Sector / Industria</label>
            <CreatableSelect value={sector||undefined} onChange={v=>setSector(v)} options={existingSectors} placeholder="Seleccionar o crear sector..."/></div>
        ...
        {step==="review"&&(<>
          ...
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:11,minWidth:700}}><thead><tr>{["Covenant","Tipo","Op.","Límite","Unidad","Valor actual","Holgura",""].map(h=><th key={h} style={{textAlign:"left",padding:"6px 8px",background:AZ,color:"#fff",fontSize:10,fontWeight:500}}>{h}</th>)}</tr></thead>
          <tbody>{detected.map(r=><CovenantRow key={r._id} r={r} isNew={r._isExtra} onUpdate={updateDetected} onRemove={removeDetected} issuer={{id:"__new__"}} sinp={sinp} btnDanger={btnDanger} allUnits={allUnits} setAllUnits={setAllUnits}/>)}</tbody></table>
          </div>
          ...
          <div style={{display:"flex",gap:8,marginTop:14}}>
            ...
            <button onClick={handleSave} disabled={loading||missingRequired.length>0} style={{...btn(missingRequired.length>0?"#ccc":AZ),flex:2}}>
...
function EditModal({issuer,onClose,onSuccess,allIssuers}){
  ...
  async function handleSave(){setLoading(true);setError("");try{const covenants=rows.filter(r=>r.name.trim()).map(r=>{const num=parseNum(r.actualVal);const lim=r.lim!==null&&r.lim!==undefined?r.lim:parseNum(r.limite);const unit=r.unidad||"x";if(isNaN(num))return{name:r.name,tipo:r.tipo,op:r.op,lim:isNaN(lim)?null:lim,limite:r.limite||"",unidad:unit,actual:null,act:null,holgura:null,actualStr:null,holguraStr:null};const holgura=r.op==="<="?lim-num:num-lim;return{name:r.name,tipo:r.tipo,op:r.op,lim:isNaN(lim)?null:lim,limite:r.limite||fmtNum(lim,unit),unidad:unit,actual:fmtNum(num,unit),act:num,holgura:isNaN(holgura)?null:holgura,actualStr:fmtNum(num,unit),holguraStr:fmtNum(holgura,unit)};});const res=await fetch("/api/issuers",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({issuerId:issuer.id,covenants,fechaEEFF,replaceAll:true})});if(!res.ok)throw new Error("Error guardando");onSuccess();}catch(e){setError(e.message);}finally{setLoading(false);}}
  ...
  return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.45)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={e=>e.target===e.currentTarget&&onClose()}>
    <div style={{background:"#fff",borderRadius:8,width:"100%",maxWidth:860,maxHeight:"90vh",overflow:"auto",boxShadow:"0 20px 60px rgba(0,0,0,0.3)"}}>
      ...
      <div style={{padding:24}}>
        ...
        <div style={{overflowX:"auto"}}>
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:11,minWidth:720}}><thead><tr>{["Nombre","Tipo","Op.","Límite","Unidad","Valor actual","Holgura",""].map(h=><th key={h} style={{textAlign:"left",padding:"6px 8px",background:AZ,color:"#fff",fontSize:10,fontWeight:500}}>{h}</th>)}</tr></thead>
        <tbody>{rows.map(r=><CovenantRow key={r._id} r={r} isNew={!issuer.covenants.find(c=>c.name===r.name)} onUpdate={updateRow} onRemove={removeRow} issuer={issuer} sinp={sinp} btnDanger={btnDanger} allUnits={allUnits} setAllUnits={setAllUnits}/>)}</tbody></table>
        </div>
        ...
        <div style={{display:"flex",gap:8,marginTop:14}}><button onClick={onClose} style={{...btn("#f0f0f0","#666"),flex:1}}>Cancelar</button><button onClick={handleSave} disabled={loading} style={{...btn(),flex:2}}>{loading?"Guardando...":"✓ Guardar cambios"}</button></div>
      </div>
    </div>
  </div>);}
...
function UploadModal({issuer,onClose,onSuccess}){
  ...
  async function handleSave(){setLoading(true);try{
    const allCovs=[...rows,...extraRows].filter(r=>r.name?.trim()).map(r=>{
      const num=parseNum(r.actualVal);const lim=r.lim!==null&&r.lim!==undefined?r.lim:parseNum(r.limite);const unit=r.unidad||"x";
  ...
  return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.45)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={e=>e.target===e.currentTarget&&onClose()}>
    <div style={{background:"#fff",borderRadius:8,width:"100%",maxWidth:step==="confirm"?880:540,maxHeight:"90vh",overflow:"auto",boxShadow:"0 20px 60px rgba(0,0,0,0.3)"}}>
      ...
      <div style={{padding:24}}>
        ...
        {step==="confirm"&&(<>
          ...
          {newlyFound.length>0&&<div style={{background:"rgba(101,169,124,0.08)",border:`1px solid ${L.ok}`,borderRadius:5,padding:"7px 12px",marginBottom:10,fontSize:12}}><strong style={{color:"rgb(40,120,70)"}}>✓ Nuevos covenants que se agregarán: </strong>{newlyFound.map(r=>r.name).join(", ")}</div>}
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
            ...
            <div style={{display:"flex",gap:8,alignItems:"center"}}>
              <div><label style={{fontSize:10,color:L.sub,marginRight:4}}>Período:</label><input style={{...sinp(90)}} value={fechaEEFF} onChange={e=>setFechaEEFF(e.target.value)}/></div>
            </div>
          </div>
          <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:11,minWidth:720,marginBottom:12}}><thead><tr>{["Covenant","Tipo","Op.","Límite","Unidad","Valor","Holgura","Fuente",""].map(h=><th key={h} style={{textAlign:"left",padding:"6px 8px",background:AZ,color:"#fff",fontSize:10,fontWeight:500}}>{h}</th>)}</tr></thead>
          <tbody>{rows.map(r=>{
            const num=parseNum(r.actualVal);const lim=r.lim!==null&&r.lim!==undefined?r.lim:parseNum(r.limite);
            const st=!isNaN(num)&&!isNaN(lim)?getStatus({...r,act:num,lim}):"na";
            const holgura=!isNaN(num)&&!isNaN(lim)?(r.op==="<="?lim-num:num-lim):null;
            const unit=r.unidad||"x";
            return(<tr key={r._id} style={{background:r._isNew?"rgba(101,169,124,0.06)":!r._found?"rgba(255,200,0,0.05)":"transparent"}}>
              ...
              <td style={{padding:"4px 7px",borderBottom:`1px solid ${L.border}`}}><button onClick={()=>removeRow(r._id)} style={{...btnDanger,padding:"3px 6px",fontSize:10}}>✕</button></td>
            </tr>);})}
          </tbody></table>
          </div>
          ...
          <div style={{display:"flex",gap:8,marginTop:12}}><button onClick={()=>{setStep("upload");setResult(null);setRows([]);setExtraRows([]);setShowPicker(false);}} style={{...btn("#f0f0f0","#666"),flex:1}}>← Volver</button><button onClick={handleSave} disabled={loading} style={{...btn(),flex:2}}>{loading?"Guardando...":"✓ Confirmar y guardar"}</button></div>
        </>)}
      </div>
    </div>
  </div>);}
...
export default function App(){
  ...
  async function loadData(){setLoading(true);try{const res=await fetch("/api/issuers");const data=await res.json();setIssuers(data);setLastUpdate(new Date().toLocaleTimeString("es-CL"));}catch(e){console.error(e);}finally{setLoading(false);}}
  useEffect(()=>{loadData();},[]);
  ...
  return(<><style>{"*{box-sizing:border-box;margin:0;padding:0}body{font-family:'Segoe UI',Arial,sans-serif}@keyframes spin{to{transform:rotate(360deg)}}"}</style>
    ...
    {editIssuer&&<EditModal issuer={editIssuer} onClose={()=>setEditIssuer(null)} onSuccess={()=>{setEditIssuer(null);loadData();}} allIssuers={issuers}/>}
